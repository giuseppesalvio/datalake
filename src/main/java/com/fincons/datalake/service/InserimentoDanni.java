...
import static com.fincons.datalake.service.Constant.FOR_MAX_RANDOM_MAX_MOVIMENTI;

@Service
public class InserimentoDanni {
  @Autowired PadatisingoliRepository paDatiSingoliRepository;
  @Autowired PaindirizzoRepository paIndirizzoRepository;
  @Autowired PasoggettoRepository paSoggettoRepository;
  @Autowired PasoggettolockRepository paSoggettoLockRepository;
  @Autowired PcruoloRepository pcRuoloRepository;
  @Autowired PcpolizzaRepository pcPolizzaRepository;
  @Autowired PcpolizzaruoloRepository pcpolizzaruoloRepository;
  @Autowired PcmovimentoRepository pcmovimentoRepository;
  ...
  public void pf(Integer ecidContraente) {
    ...
    inserimentoPolizzaE_Ruolo(codicePasoggettolock);
  }
  ...
  private void inserisciNumeroCasualeTitoliMovimenti(Integer codicePcPolizza) {
    int numeroTitoli = getRandomNumber(FOR_MAX_RANDOM_MIN_TITOLI,FOR_MAX_RANDOM_MAX_TITOLI);
    for (int i = FOR_MIN_COUNTER_TITOLI; i < numeroTitoli; i++) {
      Integer codicePgTitolo = inserimentoTitolo();
      inserisciMovimenti(codicePcPolizza, codicePgTitolo);
    }
  }

  private void inserisciMovimenti(Integer codicePcPolizza, Integer codicePgTitolo) {
    int numeroMovimenti =
        getRandomNumber(FOR_MAX_RANDOM_MIN_MOVIMENTI, FOR_MAX_RANDOM_MAX_MOVIMENTI);
    for (int j = FOR_MIN_COUNTER_MOVIMENTI; j < numeroMovimenti; j++) {
  ...
  private void inserimentoMovimentoVersione(Integer codicePcPolizza, Integer codicePgTitolo) {
    ...
    pcmovimentoRepository.save(
        getPcMovimento(
            codicePcPolizza, codicePcMovimento, codicePgTitolo, /*bannullato*/ getRandomNumber(0, 1)));
  ...
  private void inserimentoPolizzaE_Ruolo(Integer codicePasoggettolock) {
    for (int i = 1; i < 6; i++) {
  ...
  private Integer inserimentoDatiAnagraficiGiuridica(Integer ecidContraente) {
    ...
    paSoggettoRepository.save(
        getPaSoggetto(
            codicePaSoggetto, codicePaDatiSingoli, TIPO_PG_DANNI)); // 1 per fisico 2 per giuridico
    paSoggettoLockRepository.save(
        getPaSoggettoLock(
            codicePasoggettolock,
            codicePaSoggetto,
            codicePaDatiSingoli,
            IDRESIDENZA,
            0)); // codicePaIndirizzo va preso dal commander?
  ...
  private PcmovimentoEntity getPcMovimento(
      Integer codicePcPolizza,
      Integer codicePcMovimento,
      Integer codicePgTitolo,
      Integer bAnnullato) {
  ...
  private PasoggettolockEntity getPaSoggettoLock(
      Integer codicePasoggettolock,
      Integer codicePaSoggetto,
      Integer codicePaDatiSingoli,
      Integer idresidenza,
      Integer codicePaIndirizzo) {
  ...
  private PasoggettoEntity getPaSoggetto(
      Integer codicePaSoggetto, Integer codicePaDatiSingoli, Integer tipo) {
  ...
  private static PadatisingoliEntity getPadatisingoliG(
      Integer idcontraente, Integer ecidContraente) {
    return PadatisingoliEntity.builder()
        .iddatisingoli(idcontraente)
